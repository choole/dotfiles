...
then 
    ...
    do
        ...
        then
            ...
            launched=$((launched+1))
        elif [[ "$bs" = "Charging" && $launched_btr -lt 2]]
        then
            if [[ "$bat_percent" -gt 90 && "$bat_percent" -lt 100 ]]
            then
            notify-send --urgency=normal "$bat_percent% : Battery almost full!"
            break 
            fi
            launched=0
            launched_btr=$((launched_btr+1))
            notify-send --urgency=low "$bat_percent% : Charging!"
        fi
        sleep $POLL_INTERVAL
    done
fi
